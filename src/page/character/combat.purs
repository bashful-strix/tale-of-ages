module ToA.Page.Character.Combat
  ( combatCharacterPage
  ) where
...
combatCharacterPage :: Env -> Name -> Nut
combatCharacterPage env@{ characters, icon } pathChar =
  characters <&> icon <#~>
    \( chars /\
         icon_@
      ...
      let
        ...
        trs =
          traits
            ...
            <. filtered
              ( preview _name >>>
                  ( ( (\_ -> pure Zero /= (char ^? _Just <. _build <. _level))
                        && (eq (job ^? _Just <. _trait))
                    ) ||
                      (eq (cls ^? _Just <. _trait))
                  )
        ...
        D.div
          ...
          , tripanel
              ...
              , D.div
                  ...
                  , guard
                      (char # hasn't (_Just <. _build <. _level <. only Zero))
                      $ D.div
                          [ css_ [ "flex", "gap-x-2" ] ]
                      $
                        abilities
                      ^:: traversed
                      <. filtered
                        (preview _name >>> eq (job ^? _Just <. _limitBreak))
                      <. to (renderAbility icon_)
                  ]
              ]
              [ D.div
